...
test("editorial CtaBand hides feature lists and stacks action under status", () => {
  ...
  // Price secondary to heading: 26px → 28px → 30px (≤ heading, medium weight).
  assert.match(cta, /text-\[1\.625rem\]/)
  assert.match(cta, /text-\[1\.75rem\]/)
  assert.match(cta, /text-\[1\.875rem\]/)
  assert.match(cta, /font-medium leading-none/)
  assert.doesNotMatch(cta, /text-\[2\.125rem\]/)
  assert.doesNotMatch(cta, /text-5xl/)
